<x-app-layout>
    ...
    <div class="h-screen md:flex md:flex-row">
        ...
        <div class="flex w-full flex-col bg-white md:w-5/12">
            ...
            {{-- Middle --}}
            <div class="grow">
                <div class="flex items-start pl-5 gap-3 " style=" border-bottom: 1px solid var(--ig-border);">
                    {{-- <img src="{{ Str::startsWith($post->owner->image, 'https') ? $post->owner->image : asset('storage/' . $post->owner->image) }}"
                ...
                @foreach ($post->comments as $comment)
                    <div class="flex items-start px-5 py-2 gap-2">
                        <img src="{{ Str::startsWith($comment->owner->image, 'https') ? $comment->owner->image : asset('storage/' . $comment->owner->image) }}"
            ...
            <div class="post-actions">
               ...
               @livewire('like',['post'=>$post])
                <a class="action-btn" onclick="document.getElementById('comment_body').focus()"><i
                        class="far fa-comment text-2xl hover:text-gray-400 cursor-pointer mr-3"></i></a>
                <a class="action-btn"><i
                        class="far fa-paper-plane text-2xl hover:text-gray-400 cursor-pointer mr-3"></i></a>
                <a class="action-btn save"><i
                        class="far fa-bookmark text-2xl hover:text-gray-400 cursor-pointer mr-3"></i></a>
            </div>
